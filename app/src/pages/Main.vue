    ...
    <v-card :elevation="0" class="mt-2 mb-3 pa-4 text-center">
      <div class="d-flex align-center justify-center">
        ...
        <v-autocomplete
            ...
            item-text="NAME"
            return-object label="Выберите отдел "
    ...
    async fetchTasks(filter) {
      ...
      await this.callMethod(
          'tasks.task.list',
          {
            filter: {
              ...filter,
              RESPONSIBLE_ID: this.filterUser ? this.filterUser.ID : this.currentUsers.map(el => el.ID)
            },
            select: [
              'ID',
              'TITLE',
      ...
      await this.callMethod(
          'tasks.task.list',
          {
            filter: {...filter, ACCOMPLICE: this.filterUser ? this.filterUser.ID : this.currentUsers.map(el => el.ID)},
            select: [
              'ID',
              'TITLE',
              'STATUS',
